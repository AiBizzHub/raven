...
function App() {
  ...
  // We need to pass sitename only if the Framework version is v1 or above.

  const getSiteName = () => {
    // @ts-ignore
    if (window.frappe?.boot?.versions?.frappe && (window.frappe.boot.versions.frappe.startsWith('1') || window.frappe.boot.versions.frappe.startsWith('2'))) {
      // @ts-ignore
